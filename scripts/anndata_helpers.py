...
def load_and_build_anndata(file_path: str) -> ad.AnnData:
    ...
    # --- TEMPORARY: drop rows where 'Nucleus/Cell area ratio' is NaN ---
    if "Nucleus/Cell area ratio" in df.columns:
        n_na = df["Nucleus/Cell area ratio"].isna().sum()
        if n_na > 0:
            print(f"Dropping {n_na} rows with NaN in 'Nucleus/Cell area ratio'")
            df = df.dropna(subset=["Nucleus/Cell area ratio"])
