...
def add_intensity_tiers(
    ...
    """
    Add low/mid/high intensity tiers for a marker, restricted to marker_pos cells.
    Adds intensity tier annotations to adata.obs in place.
    """

    pos_col = f"{marker}_pos"
    if pos_col not in adata.obs:
        raise ValueError(f"{pos_col} not found in adata.obs")

    var_candidates = [v for v in adata.var_names if v.endswith(marker)]
    if len(var_candidates) != 1:
        raise ValueError(f"Marker {marker} not uniquely found in adata.var_names")

    var_name = var_candidates[0]

    x = np.asarray(adata[:, var_name].X).flatten()
    pos_mask = adata.obs[pos_col].values
    x_pos = x[pos_mask]

    if len(x_pos) == 0:
        raise ValueError(f"No {marker}_pos cells found")

    low_thr = np.quantile(x_pos, low_q)
    high_thr = np.quantile(x_pos, high_q)

    adata.obs[f"{marker}_low"] = pos_mask & (x <= low_thr)
    adata.obs[f"{marker}_mid"] = pos_mask & (x > low_thr) & (x < high_thr)
    adata.obs[f"{marker}_high"] = pos_mask & (x >= high_thr)

    # Diagnostics
    n_total = adata.n_obs
    n_pos = int(pos_mask.sum())
    n_low = int(adata.obs[f"{marker}_low"].sum())
    n_mid = int(adata.obs[f"{marker}_mid"].sum())
    n_high = int(adata.obs[f"{marker}_high"].sum())

    print(f"\n{marker} intensity tiers (restricted to {marker}_pos cells)")
    print(f"  low_q  = {low_q:.2f}  -> threshold = {low_thr:.4f}")
    print(f"  high_q = {high_q:.2f} -> threshold = {high_thr:.4f}")
    print("Counts:")
    print(f"  total cells:   {n_total}")
    print(f"  {marker}_pos:  {n_pos}")
    if n_pos > 0:
        print(f"  {marker}_low:  {n_low}  ({100 * n_low / n_pos:.1f}%)")
        print(f"  {marker}_mid:  {n_mid}  ({100 * n_mid / n_pos:.1f}%)")
        print(f"  {marker}_high: {n_high}  ({100 * n_high / n_pos:.1f}%)")

    # Store thresholds + counts for provenance

    adata.uns.setdefault("intensity_thresholds", {})[marker] = {
        "low_q": low_q,
        "high_q": high_q,
    ...
    if plot:
        ...
        sns.histplot(x_pos, bins=40, stat="density")
        plt.axvline(low_thr, color="blue", linestyle="--", label="low")
        plt.axvline(high_thr, color="red", linestyle="--", label="high")
        plt.title(f"{marker} intensity tiers (pos cells only)")
        plt.legend()
        plt.tight_layout()
        plt.show()
