...
This module  holds display and bookkeeping metadata shared across the
IHOPE notebooks: which sample is which donor/tissue,
what order things should be plotted in,
what each cell type should be called and colored.
...
# filename, display sample name. The sample
# comparison/SVG export notebooks base donor_map and tissue_map (below)
# on these display names, since that's what load_celltype_summaries
...
# DONOR_MAP/TISSUE_MAP above, but based on the underscore-joined
...
# Immune only with no unannotated cells.
